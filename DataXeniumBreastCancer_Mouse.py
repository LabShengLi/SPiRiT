...
def load_data(data_set, ifNorm = False, ifSample = True):
    ...
    gene_intersect_df = pd.read_csv(code_dir+'py/XeniumExperiments/xenium_genes_intersection_mouse.csv', index_col = 0)
    ...
    if ifSample:
        rep_num = 320000
        #rep_num = 160000
        rand_indx = np.random.choice(len(filtered_files), rep_num, replace = False)
        fl = np.array(filtered_files)[rand_indx]
    else:
        rep_num = 250000
        rand_indx = np.random.choice(len(filtered_files), rep_num, replace = False)
        fl = np.array(filtered_files)[rand_indx]
